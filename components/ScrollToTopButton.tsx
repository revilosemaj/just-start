export default function ScrollToTopButton({
  ...
  return (
    <div className="fixed bottom-6 right-6 sm:bottom-8 sm:right-8 z-40">
      ...
      <button
        onClick={scrollToTop}
        className={`group relative bg-[var(--primary)] text-white p-3 sm:p-4 rounded-full shadow-lg hover:shadow-2xl hover:shadow-[#00ADB5]/50 transition-all duration-300 overflow-visible ${
          showScrollTop
            ? 'opacity-100 translate-y-0'
            : 'opacity-0 translate-y-4 pointer-events-none'
        } ${isRocketLaunching ? 'animate-[rocketLaunch_1.5s_ease-out_forwards]' : ''}`}
      ...
      >
        ...
        <div className="relative z-10">
          <svg
            className={`w-5 h-5 sm:w-6 sm:h-6 transition-transform duration-300 text-white dark:text-black ${
              isRocketLaunching ? 'rotate-[-45deg] scale-110' : 'group-hover:scale-110'
            }`}
            fill="none"
            strokeLinecap="round"
